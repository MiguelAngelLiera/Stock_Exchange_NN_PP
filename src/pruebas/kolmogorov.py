...
def prueba_KS(m1,m2,modo_UD):
    M1 = np.load(f'distribuciones/ACTINVRB/{m1}.npy')
    M2 = np.load(f'distribuciones/ACTINVRB/{m2}.npy')
    ...
    plt.text(0.75, 0.1, f"KS: {round(ts_ks_statistic,4)}, Valor p: {round(ts_ks_p_value,18)}", fontsize=12, ha='center', va='center', transform=plt.gca().transAxes)
    plt.text(0.75, 0.05, f"KS: {round(os_ks_statistic,4)}, Valor p: {round(os_ks_p_value,18)}", fontsize=12, ha='center', va='center', transform=plt.gca().transAxes)

    plt.plot(M1, color = '#451952', label = f'{m1}') #Señal original
    plt.plot(M2, color='#AE445A', label = f'{m2}') #Señal predicha
